...
class Parser:
	...
	def parse2(self,corpus):
		docs = list([ d.text for d in corpus.documents ])
		print(now())
		for doctokens in self.nlp.pipe( docs , batch_size=10, n_threads=8):
			continue
			#print(doctokens)
			#for t in doctokens:
			#	print((t.i,t.text,t.pos_,t.dep_,t.head.i,t.is_sent_start))
			#break
		print(now())
	...
	def parse(self,corpus):
		...
		for d,doctokens in zip(corpus.documents,self.nlp.pipe( [ d.text for d in corpus.documents ] , batch_size=10, n_threads=-1)):
		#for d in corpus.documents:
		#	doctokens = self.nlp(d.text)
			#continue
			...
			for sentence in sentenceSplit(doctokens):
				tokens = []
				for t in sentence:
					token = kindred.Token(t.text,None,None,t.idx,t.idx+len(t.text))
					tokens.append(token)
